...
const CartItems = ({ showQuantity = false }: { showQuantity?: boolean }) => {
    ...
    return (
        <>
            {items.length === 0 ?
                <p className={`text-center mt-3 ${showQuantity && 'text-2xl'}`}>No items in cart</p>
                : (
                    <>
                        <div className={`flex flex-col gap-4 my-5 ${showQuantity ? 'text-xl' : 'text-lg'}`}>
                            {items.map(item => (
                                <div key={item.productId} className={`flex justify-between ${showQuantity ? 'py-4' : 'py-2'} items-center border-b border-gray-200`}>
